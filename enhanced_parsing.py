...
class EnhancedMessageParser:
    def __init__(self):
        ...
        # --- UPDATED: ELECTRICITY BILL PATTERNS ---
        self.electricity_indicators = [
            r'\belectricity\b', r'\bconsumer\s*no\b', r'\bkno\b', r'\bkwh\b',
            r'\bunits\b', r'\bvidyut\b', r'\bdiscom\b', r'\bavvnl\b',
            r'\bmppkvvcl\b', r'\bjvvnl\b', r'\bdhbvnl\b', r'\bbill\s*amt\b',
            r'bill\s*generation', r'\burjapay\b', r'edani',
            # ENHANCED: More comprehensive indicators
            r'\bbses\b', r'mp\s*paschim\s*kshetra', r'ca\s*number', r'connection\s*no',
            r'\belectricity\s*bill\b', r'\bpower\s*bill\b', r'\benergy\s*bill\b',
            r'\belectricity\s*payment\b', r'\bpower\s*payment\b', r'\bupi\s*ref\b',
            r'\bmandate\b.*\belectricity\b', r'\btxn\s*id\b.*\belectricity\b'
        ]

        self.electricity_bill_amount_patterns = [
            # PRIORITY: Most specific patterns first
            r'bill\s*for\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',  # "bill for Rs.8320.00"
            r'electricity\s*bill\s*for\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',  # "electricity bill for Rs.X"
            r'payment\s*of\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)\s*(?:against|for|on|towards)',  # "payment of Rs 381 on date"
            r'received\s*cash\s*amount\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',  # "Received Cash Amount Rs 381"
            r'debited\s*with\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)\s*towards',  # "debited with Rs. 395.00 towards"
            
            # Existing patterns (keeping for backward compatibility)
            r'rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)\s*(?:/-)?\s*against',
            r'bill\s*of\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'inr\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'bill\s*amt[:\s]*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'bill\s*amt\s*after\s*due[:\s]*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'amount\s*of\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'electricity\s*charges\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            r'cash\s*amount\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',
            
            # NEW: Transaction and payment specific patterns
            r'rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)\s*.*?(?:electricity|power)\s*bill',  # "Rs.90 for MP Paschim Kshetra - Indore Electricity bill"
            r'for\s*rs\.?\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)\s*.*?(?:electricity|power)',  # Generic "for Rs.X ... electricity"
        ]

        self.electricity_due_date_patterns = [
            # PRIORITY: Enhanced patterns for various date formats
            r'due\s*on\s*(\d{1,2}\s+[a-z]{3,9}\s+\d{4})',  # "due on 11 October 2019"
            r'is\s*due\s*on\s*(\d{1,2}\s+[a-z]{3,9}\s+\d{4})',  # "is due on 11 October 2019"
            r'mandate\s*is\s*set\s*for\s*(\d{1,2}[-/.][\w]{3,9}[-/. ]\d{2,4})',  # "mandate is set for 22-Jul-24"
            r'set\s*for\s*(\d{1,2}[-/.][\w]{3,9}[-/. ]\d{2,4})',  # "set for 22-Jul-24"
            r'on\s*date\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})',  # "on date 27/04/24"
            
            # Existing patterns
            r'due\s*on\s*(\d{1,2}[-/.][\w]{3,9}[-/. ]\d{2,4})',  # 25-May-2022
            r'due\s*on\s*(\d{1,2}[-/. ]\d{1,2}[-/. ]\d{2,4})',    # 12/08/2024, 09.08.2024
            r'due\s*date[:\s]*(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})',  # DUE DATE:10/08/2022
        ]

        self.electricity_units_patterns = [
            r'consumption\s*of\s*(\d+(?:\.\d+)?)\s*kwh',
            r'for\s*(\d+(?:\.\d+)?)\s*units',
            # NEW: Additional unit patterns
            r'(\d+(?:\.\d+)?)\s*kwh\s*consumed',
            r'(\d+(?:\.\d+)?)\s*units\s*consumed',
        ]

        self.electricity_consumer_number_patterns = [
            # PRIORITY: Enhanced patterns with better flexibility
            r'ca\s*number\s*(\d{8,15})\b',  # "CA Number 151998587"
            r'connection\s*no\.?\s*([A-Z]?\d{8,15})\b',  # "connection no. N3147002125"
            r'consumer\s*(?:number|no)\s*(\d{8,15})\b',  # "Consumer number 3147002125"
            r'electricity\s*bill\s*\(\s*([A-Z0-9]{8,20})\s*\)',  # "Electricity bill (N3781003484)"
            r'bill\s*\(\s*([A-Z0-9]{8,20})\s*\)',  # "bill (N3781003484)"
            r'for\s*consumer\s*number\s*(\d{8,15})\b',  # "for Consumer number 3147002125"
            
            # Existing patterns (keeping for backward compatibility)
            r'kno\s*[:\s]*\(?([A-Z0-9]{10,20})\)?\b',
            r'account\s*number[:\s]*\(?([A-Z0-9]{8,20})\)?\b',
            r'ac\s*no\.?\s*\(?([A-Z0-9/]{6,15})\)?\b',
            r'consumer\s*(?:number|no)\s*[:\s]*\(?([A-Z0-9]{8,20})\)?\b',
            r'ca\s*number\s*\(?([A-Z0-9]{8,20})\)?\b',
            r'connection\s*no\.?\s*\(?([A-Z0-9]{8,20})\)?\b',
        ]

        self.electricity_provider_patterns = {
            'AVVNL': [r'\bavvnl\b'],
            'JVVNL': [r'\bjvvnl\b', r'jaipur\s*vidyut'],
            'DHBVNL': [r'\bdhbvnl\b'],
            'West Bengal Electricity': [r'west\s*bengal\s*electricity'],
            'Bajaj Pay': [r'bajaj\s*pay'],
            'EDANI': [r'\bedani\b'],
            'MPPKVVCL': [r'\bmppkvvcl\b', r'mp\s*paschim\s*kshetra', r'mppkvvcl.*indore'],  # Enhanced
            'MPPKVVCL-JBP': [r'mppkvvcl-jbp'],
            'BSES': [r'\bbses\b'],
            # NEW: Additional providers
            'BOI UPI': [r'boi\s*upi', r'bank\s*of\s*india\s*upi'],
            'PhonePe': [r'\bphonepe\b'],
            'PayTM': [r'\bpaytm\b'],
        }

        self.electricity_status_patterns = {
            # ENHANCED: More comprehensive status detection
            'paid': [
                r'thank\s*you\s*for\s*(?:making\s*)?payment',
                r'received\s*(?:cash\s*)?amount',
                r'we\s*have\s*received',
                r'payment.*?received',
                r'successfully\s*(?:paid|done)',
            ],
            'due': [
                r'is\s*due\s*on',
                r'due\s*on',
                r'due\s*reminder',
                r'upcoming\s*mandate',
                r'will\s*be\s*debited',
                r'account\s*will\s*be\s*debited',
            ],
            'generated': [
                r'has\s*been\s*generated',
                r'bill\s*generated',
            ],
            'payment_failed': [
                r'has\s*failed',
                r'payment\s*failed',
                r'payment.*?has\s*failed',
        ...
        self.strong_exclusion_patterns = [
            ...
            r'call\s*delivery\s*manager\s*\d{10}',
            # NEW: Exclusions for electricity bills to prevent misclassification as OTP
            r'\belectricity\s*bill\b',
            r'\bca\s*number\b',
            r'\bconsumer\s*no\b',
            r'\bconnection\s*no\b',
        ]
    ...
    def _compile_patterns(self):
        ...
        # --- NEW: Electricity pattern compilation ---
        self.compiled_electricity_indicators = [re.compile(p, re.IGNORECASE) for p in self.electricity_indicators]
        self.compiled_electricity_bill_amount_patterns = [re.compile(p, re.IGNORECASE) for p in self.electricity_bill_amount_patterns]
        self.compiled_electricity_due_date_patterns = [re.compile(p, re.IGNORECASE) for p in self.electricity_due_date_patterns]
        self.compiled_electricity_units_patterns = [re.compile(p, re.IGNORECASE) for p in self.electricity_units_patterns]
        self.compiled_electricity_consumer_number_patterns = [re.compile(p, re.IGNORECASE) for p in self.electricity_consumer_number_patterns]
        self.compiled_electricity_provider_patterns = {}
        for provider, patterns in self.electricity_provider_patterns.items():
            self.compiled_electricity_provider_patterns[provider] = [re.compile(p, re.IGNORECASE) for p in patterns]
        self.compiled_electricity_status_patterns = {}
        for status, patterns in self.electricity_status_patterns.items():
            self.compiled_electricity_status_patterns[status] = [re.compile(p, re.IGNORECASE) for p in patterns]
    ...
    def normalize_date(self, date_str: str) -> str:
        """UPDATED: Normalize various date formats to a standard format with better handling"""
        ...
        # NEW: Handle DD Month YYYY (e.g., 11 October 2019)
        dd_month_yyyy_match = re.match(r"(\d{1,2})\s+([a-z]{3,9})\s+(\d{4})", date_str, re.IGNORECASE)
        if dd_month_yyyy_match:
            day = dd_month_yyyy_match.group(1).zfill(2)
            month_name = dd_month_yyyy_match.group(2).title()
            year = dd_month_yyyy_match.group(3)
            try:
                ...
                try:
                    ...
                    return f"{day}-{month_name[:3]}-{year}"
...
# IMPROVED: Enhanced extract_delivery_date method  
    ...
    # --- UPDATED: ELECTRICITY BILL PARSING METHODS ---
    def extract_electricity_bill_amount(self, text: str) -> Optional[str]:
        """Extract bill amount from electricity messages."""
        for pattern in self.compiled_electricity_bill_amount_patterns:
            match = pattern.search(text)
            if match:
                amount = match.group(1).replace(',', '')
                try:
    ...
    def extract_electricity_due_date(self, text: str) -> Optional[str]:
        """Extract due date from electricity messages."""
        for pattern in self.compiled_electricity_due_date_patterns:
            match = pattern.search(text)
            if match:
                date_str = match.group(1)
                return self.normalize_date(date_str)
    ...
    def extract_electricity_units(self, text: str) -> Optional[str]:
        """Extract units consumed from electricity messages."""
        for pattern in self.compiled_electricity_units_patterns:
            match = pattern.search(text)
            if match:
                return match.group(1)
        return None

    def extract_electricity_consumer_number(self, text: str) -> Optional[str]:
        """Extract consumer/account number from electricity messages."""
        for pattern in self.compiled_electricity_consumer_number_patterns:
            match = pattern.search(text)
            if match:
                return match.group(1)
        # Fallback for numbers in parentheses in a confirmed electricity message
        if any(p.search(text.lower()) for p in self.compiled_electricity_indicators):
            match = re.search(r'\(\s*([A-Z0-9]{8,20})\s*\)', text)
            if match:
                return match.group(1)
        return None

    def extract_electricity_provider(self, text: str, sender_name: str = "") -> Optional[str]:
        """Extract electricity service provider name."""
        combined_text = f"{text.lower()} {sender_name.lower()}"
        for provider, patterns in self.compiled_electricity_provider_patterns.items():
            if any(p.search(combined_text) for p in patterns):
                return provider
        return None

    def determine_electricity_bill_status(self, text: str) -> str:
        """Determine the status of the electricity bill."""
        text_lower = text.lower()
        if any(p.search(text_lower) for p in self.compiled_electricity_status_patterns['paid']):
            return 'paid'
        if 'payment_failed' in self.compiled_electricity_status_patterns and any(p.search(text_lower) for p in self.compiled_electricity_status_patterns['payment_failed']):
            return 'payment_failed'
        if any(p.search(text_lower) for p in self.compiled_electricity_status_patterns['due']):
            return 'due'
        if any(p.search(text_lower) for p in self.compiled_electricity_status_patterns['generated']):
            return 'generated'
        return 'unknown'

    def calculate_electricity_confidence_score(self, text: str, sender_name: str = "") -> int:
        """Calculate confidence score for electricity bill messages."""
        score = 0
        text_lower = text.lower()
        combined_text = f"{text_lower} {sender_name.lower()}"

        # Strong boost for primary indicators
        indicator_count = sum(1 for p in self.compiled_electricity_indicators if p.search(combined_text))
        score += indicator_count * 15

        # Score based on extracted entities
        if self.extract_electricity_bill_amount(text):
            score += 25
        if self.extract_electricity_due_date(text):
            score += 20
        if self.extract_electricity_consumer_number(text):
            score += 20
        if self.extract_electricity_provider(text, sender_name):
            score += 15
        if self.extract_electricity_units(text):
            score += 10

        return min(100, score)

    def parse_electricity_message(self, message: str, sender_name: str = "") -> Dict:
        """Parse electricity bill information from a message."""
        clean_message = self.clean_text(message)
        confidence_score = self.calculate_electricity_confidence_score(clean_message, sender_name)
        ...
        if confidence_score >= 50:
            result = {
                ...
                'confidence_score': confidence_score,
                'bill_amount': self.extract_electricity_bill_amount(clean_message),
                'due_date': self.extract_electricity_due_date(clean_message),
                'units_consumed': self.extract_electricity_units(clean_message),
                'consumer_number': self.extract_electricity_consumer_number(clean_message),
                'service_provider': self.extract_electricity_provider(clean_message, sender_name),
                'bill_status': self.determine_electricity_bill_status(clean_message),
                'payment_link': self.extract_payment_link(clean_message),
                'raw_message': message,
            }
            return result
        ...
        return {
            'status': 'rejected',
            'message_type': 'electricity',
            'reason': 'Message did not meet the confidence threshold for an electricity bill.',
            'confidence_score': confidence_score,
            'message_preview': clean_message[:100],
        }
    ...
    def parse_single_message(self, message: str, sender_name: str = "", message_type: str = "auto") -> Dict:
        ...
        if message_type == "auto":
            ...
            # PRIORITY 4: Check for Electricity Bill (Specific keywords)
            electricity_score = self.calculate_electricity_confidence_score(clean_message, sender_name)
            if electricity_score >= 50:
                return self.parse_electricity_message(message, sender_name)

            # PRIORITY 5: Check for e-commerce only with strong indicators
    ...
    def process_csv_file(self, input_file: str, output_file: str = None, message_type: str = "auto") -> Dict:
        ...
        print("Enhanced Message Parser v14.1 - Electricity FIXED - Analyzing Messages")
        print("=" * 90)
        ...
        ecommerce_messages = [msg for msg in parsed_messages if msg.get('message_type') == 'ecommerce']
        electricity_messages = [msg for msg in parsed_messages if msg.get('message_type') == 'electricity'] 
        ...
        results = {
            'metadata': {
                ...
                'ecommerce_messages_found': len(ecommerce_messages),
                'electricity_messages_found': len(electricity_messages), 
                ...
                'parser_version': '14.1_electricity_fixed'
            },
            'summary_statistics': {
                ...
                'ecommerce_stats': self.generate_ecommerce_summary_stats(ecommerce_messages),
                'electricity_stats': self.generate_electricity_summary_stats(electricity_messages)
            },
            ...
            'ecommerce_messages': ecommerce_messages,
            'electricity_messages': electricity_messages, 
        ...
        if output_file is None:
            ...
            output_file = f"{base_name}_parsed_messages_electricity_fixed.json"
    ...
    # NEW: Electricity summary statistics
    def generate_electricity_summary_stats(self, electricity_messages: List[Dict]) -> Dict:
        """Generate summary statistics for Electricity Bill messages."""
        if not electricity_messages:
            return {}

        providers = [msg.get('service_provider') for msg in electricity_messages if msg.get('service_provider')]
        provider_counts = {}
        for provider in providers:
            provider_counts[provider] = provider_counts.get(provider, 0) + 1

        statuses = [msg.get('bill_status') for msg in electricity_messages if msg.get('bill_status')]
        status_counts = {}
        for status in statuses:
            status_counts[status] = status_counts.get(status, 0) + 1
            
        amounts = []
        for msg in electricity_messages:
            amount_str = msg.get('bill_amount')
            if amount_str:
                try:
        ...
        confidence_scores = [msg.get('confidence_score', 0) for msg in electricity_messages]
        avg_confidence = sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0
        ...
        return {
            'total_count': len(electricity_messages),
            'distributions': {
            ...
            'quality_metrics': {
                'average_confidence_score': round(avg_confidence, 2),
                'messages_with_bill_amount': sum(1 for msg in electricity_messages if msg.get('bill_amount')),
                'messages_with_due_date': sum(1 for msg in electricity_messages if msg.get('due_date')),
                'messages_with_consumer_number': sum(1 for msg in electricity_messages if msg.get('consumer_number')),
            }
        }
    ...
    def display_parsing_summary(self, results: Dict):
        ...
        print("" + "="*90)
        print("ENHANCED MESSAGE PARSING RESULTS SUMMARY v14.1 (Electricity FIXED)")
        print("="*90)
        ...
        print(f"  - E-commerce Messages Found: {metadata['ecommerce_messages_found']:,}")
        print(f"  - Electricity Messages Found: {metadata['electricity_messages_found']:,}")
        ...
        # NEW: Display Electricity Summary
        if electricity_stats and electricity_stats.get('total_count', 0) > 0:
            print("\n" + "="*60)
            print("ELECTRICITY BILL MESSAGES SUMMARY")
            print("="*60)
            distributions = electricity_stats.get('distributions', {})
            print("Top Providers:")
            for provider, count in list(distributions.get('top_providers', {}).items())[:5]:
                percentage = (count / electricity_stats['total_count']) * 100
                print(f"  {provider}: {count:,} ({percentage:.1f}%)")
            print("Bill Status Distribution:")
            for status, count in distributions.get('status_types', {}).items():
                percentage = (count / electricity_stats['total_count']) * 100
    ...
    def interactive_message_analyzer(self):
        ...
        print("Interactive Message Analyzer v14.1 (Electricity FIXED)")
        print("=" * 70)
        print("Parsing for OTP, EMI, Challan, Transportation, EPF, E-commerce & Electricity messages")
        ...
        while True:
            ...
            message_type = input("Message type (otp/emi/challan/transportation/epf/ecommerce/electricity/auto) [auto]: ").strip().lower()
            ...
            if result['status'] == 'parsed':
                ...
                elif result['message_type'] == 'electricity':
                    print(f"Bill Amount: Rs.{result.get('bill_amount')}")
                    print(f"Due Date: {result.get('due_date')}")
                    print(f"Units Consumed: {result.get('units_consumed')}")
                    print(f"Consumer No.: {result.get('consumer_number')}")
                    print(f"Provider: {result.get('service_provider')}")
                    print(f"Bill Status: {result.get('bill_status')}")
...
if __name__ == "__main__":
    ...
    # Test the new electricity bill examples
    print("Testing Electricity Bill parsing with examples:")
    print("="*70)
    
    electricity_tests = [
        ("Your BSES electricity bill for Rs.8320.00 for CA Number 151998587 is due on 11 October 2019. Pay at http://amzn.in/d/9bT07IO Unsubscribe at http://amzn.in/d/8GLuZ32", "AMAZON"),
        ...
        ("Dear Consumer, Received Cash Amount Rs 381  - for Electricity Bill payment For Consumer number 3147002125  by MU3365101U16290 Through D2D Mode , MPPKVVCL, Indore", "MPPKCL"),
        ("BOI UPI - Your upcoming Mandate is set for 22-Jul-24 Your account will be debited with Rs. 395.00 towards MP Paschim Kshetra Electricity Bill for the PTMfcb78057645eb99b98267bc6cf2a7@paytm (UPI Ref - 420289789470). If Mandate is paused the execution for the same will not happen.", "BOIIND"),
        ("Payment of Rs.90 for MP Paschim Kshetra - Indore Electricity bill (N3781003484) via PhonePe  (Txn Id: NX22031114455275150916641) has failed. If deducted, it will be refunded within 7 - 9 working days.", "PHONPE"),
    ]
    
    for i, (msg, sender) in enumerate(electricity_tests):
        result = parser.parse_single_message(msg, sender, "auto")
        print(f"\nElectricity Test {i+1} Result:")
        print(json.dumps(result, indent=2))
    
    print("\n" + "="*70)
    print("Enhanced Parser v14.1 - Electricity parser fixed!")
    print("="*70)
